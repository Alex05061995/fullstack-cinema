...
export class RatingService {
    ...
    async getMovieValueByUser(movieId: Types.ObjectId, userId: Types.ObjectId) {
        return await this.ratingModel.findOne({movieId, userId}).select('value').exec().then(data => data ? data.value : 0)
    }
    ...
    async setRating(userId: Types.ObjectId, dto: SetRatingDto) {
        ...
        const avrageRating = await this.averageRatingByMovie(movieId);

        await this.movieService.updateRating(movieId, avrageRating);
